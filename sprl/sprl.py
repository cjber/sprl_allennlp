...
from sprl.sprl_scorer import SprlEvalScorer
...
def write_conll_formatted_tags_to_file(
    ...
    """
    indicator_only_sentence = ["-"] * len(sentence)
    if indicator_index is not None:
        indicator_only_sentence[indicator_index] = sentence[indicator_index]

    for word, predicted, gold in zip(
        indicator_only_sentence, conll_formatted_predictions, conll_formatted_gold_labels
    ):
...
class SemanticRoleLabeler(Model):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.span_metric = SprlEvalScorer(ignore_classes=["U-SI",
                                                          "B-SI",
                                                          "I-SI",
                                                          "L-SI"])
        ...
        check_dimensions_match(
            ...
            "text embedding dim + indicator embedding dim",
            "encoder input dim",
        )
        initializer(self)
    ...
    def forward(  # type: ignore
        ...
        embedded_indicator = self.binary_feature_embedding(indicator.long())
        # Concatenate the verb feature onto the embedded text. This now
        # has shape (batch_size, sequence_length, embedding_dim + binary_feature_dim).
        embedded_text_with_indicator = torch.cat(
            [embedded_text_input, embedded_indicator], -1
        )
        batch_size, sequence_length, _ = embedded_text_with_indicator.size()

        encoded_text = self.encoder(embedded_text_with_indicator, mask)
        ...
        if tags is not None:
            ...
            if not self.ignore_span_metric and self.span_metric is not None and not self.training:
                batch_indicator_indices = [
                    example_metadata["indicator_index"] for example_metadata in metadata
                ]
                batch_sentences = [example_metadata["words"]
        ...
        words, indicators = zip(
            *[(x["words"], x["indicator"]) for x in metadata])
        if metadata is not None:
            output_dict["words"] = list(words)
            output_dict["indicator"] = list(indicator)
        return output_dict
...
def write_to_conll_eval_file(
    ...
    write_bio_formatted_tags_to_file(
        prediction_file, gold_file, indicator_index, sentence, prediction, gold_labels
    )
